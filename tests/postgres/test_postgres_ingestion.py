...
def test_option_snapshot_session_includes_listed_option_close_window() -> None:
    assert market_session(datetime(2026, 7, 10, 20, 14, tzinfo=UTC)) == "regular"
    assert market_session(datetime(2026, 7, 10, 20, 16, tzinfo=UTC)) == "afterhours"


def test_option_normalization_clears_incomplete_standard_contract_claim() -> None:
    normalized = normalize_option_row({
...
def test_latest_option_snapshot_by_symbol_uses_bounded_run_manifests(
    ...
    ticket_at = second_at + timedelta(minutes=15)
    ticket = repository.start_run("robinhood", "option_quotes", started_at=ticket_at)
    repository.store_option_snapshot(ticket, source_id="robinhood", observed_at=ticket_at,
        market_session="regular", universe="paper-tickets", rows=[])
    repository.finish_run(ticket, "succeeded", summary={"symbols_requested": ["NVDA"]})
