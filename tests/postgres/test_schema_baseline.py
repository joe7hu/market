...
from migrations.baseline_contract import BASELINE_SCHEMA_HASHES, LEGACY_REVISION, LEGACY_SCHEMA_HASHES
...
def test_baseline_matches_verified_schema_and_application_queries(migrated_postgres_dsn):
...
def test_adoption_preserves_user_records_and_secret(migrated_postgres_dsn):
    with psycopg.connect(migrated_postgres_dsn) as connection:
    ...
    legacy_shape(migrated_postgres_dsn)
...
def test_adoption_rejects_drift_before_changing_anything(migrated_postgres_dsn):
    legacy_shape(migrated_postgres_dsn)
    with psycopg.connect(migrated_postgres_dsn) as connection:
...
def test_adoption_failure_rolls_back_and_can_retry(migrated_postgres_dsn, monkeypatch):
...
def test_adoption_rejects_unsafe_privileges_and_owners(migrated_postgres_dsn, mutation):
    legacy_shape(migrated_postgres_dsn)
    with psycopg.connect(migrated_postgres_dsn) as connection:
...
def test_adoption_rejects_unsafe_role_membership(migrated_postgres_dsn):
    legacy_shape(migrated_postgres_dsn)
    with psycopg.connect(migrated_postgres_dsn) as connection:
