...
def test_job_intervals_default_to_ibkr_source(monkeypatch) -> None:
    ...
    assert intervals["update_ibkr_options"] == 120
    assert intervals["refresh_options_radar_signal_ibkr"] == 60
...
def test_job_intervals_ignore_invalid_env(monkeypatch) -> None:
    ...
    intervals = scheduler.job_intervals()
    assert intervals["update_ibkr_options"] == 3600  # default
    assert intervals["refresh_options_radar_signal_ibkr"] == 900  # default
...
def test_signal_loop_always_scheduled(monkeypatch) -> None:
    ...
    intervals = scheduler.job_intervals()
    assert intervals["refresh_options_radar_signal_ibkr"] == 900
    assert intervals["update_ibkr_options"] == 3600
...
def test_learning_refresh_can_be_disabled(monkeypatch) -> None:
    ...
    assert "refresh_options_radar_signal_ibkr" in intervals  # fast loop stays
...
def test_dispatch_invokes_run_refresh_job(monkeypatch) -> None:
    ...
    asyncio.run(scheduler._dispatch("refresh_options_radar_signal_ibkr", "db", "config.yaml"))

    assert calls == [("refresh_options_radar_signal_ibkr", "db", "config.yaml")]
