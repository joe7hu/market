...
from investment_panel.jobs.update_ibkr_options import ibkr_status
...
def test_ibkr_status_does_not_report_green_when_gateway_offline() -> None:
    ...
    assert ibkr_status(["ibkr_connect_failed"], 0) == "gateway_offline"
    assert ibkr_status(["RBLX:reqId timeout"], 0) == "error"
    assert ibkr_status(["RBLX:reqId timeout"], 120) == "partial"
    assert ibkr_status([], 120) == "ok"
...
def test_select_leap_call_strikes_targets_otm_band() -> None:
    ...
    # zone, while the lottery sleeve gets its own budget out toward 3x spot.
    ...
    assert len(out) > 6
    assert len([strike for strike in out if strike <= 160.0]) == 6
