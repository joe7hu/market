"""Specialize current-price reads for bounded instrument sets.
...
def upgrade() -> None:
    # A SQL function with ``p_ids IS NULL OR ...`` can choose a generic plan
    # and scan all facts even for one ticker.  The bounded implementation has
    # no nullable filter branch, so PostgreSQL can use the fact indexes.
    op.execute(
        """
        CREATE FUNCTION raw.current_price_for_instruments(
            p_as_of TIMESTAMPTZ,
...
def downgrade() -> None:
    op.execute("DROP FUNCTION IF EXISTS raw.current_price_for_instruments(TIMESTAMPTZ, BIGINT[])")
