...
def upgrade() -> None:
    op.execute(
        ...
        CREATE TABLE raw.market_observation (
            ...
            CONSTRAINT ck_market_observation_status CHECK (status IN ('AVAILABLE','MISSING_SOURCE','MISSING_HISTORY','CONFLICTED','FALLBACK','UNSUPPORTED')),
            CONSTRAINT ck_market_observation_clocks CHECK (available_at >= observed_at OR status <> 'AVAILABLE')
        );
    ...
    op.execute(
        """
        GRANT SELECT ON raw.market_observation TO market_app;
...
def downgrade() -> None:
    op.execute(
        """
        REVOKE SELECT ON raw.market_observation FROM market_app;
