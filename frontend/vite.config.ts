import { defineConfig, type Plugin } from "vite";
import react from "@vitejs/plugin-react";
import tailwindcss from "@tailwindcss/vite";
import { execFileSync } from "node:child_process";
import { fileURLToPath, URL } from "node:url";

const apiProxyTarget = process.env.MARKET_API_PROXY_TARGET ?? "http://127.0.0.1:8010";
const frontendBuild = process.env.MARKET_FRONTEND_BUILD?.trim() || (() => {
  try {
    return execFileSync("git", ["rev-parse", "--short", "HEAD"], { encoding: "utf8" }).trim();
  } catch {
    return "unknown";
  }
})();

const buildIdentityHeader = (): Plugin => ({
  name: "market-build-identity",
  configureServer(server) {
    server.middlewares.use((_request, response, next) => {
      response.setHeader("X-Market-Frontend-Build", frontendBuild);
      next();
    });
  },
});

export default defineConfig({
  plugins: [buildIdentityHeader(), react(), tailwindcss()],
  define: { __MARKET_FRONTEND_BUILD__: JSON.stringify(frontendBuild) },
  resolve: {
    alias: {
      "@": fileURLToPath(new URL("./src", import.meta.url)),
    },
  },
  build: {
    outDir: "dist",
    emptyOutDir: true,
    rollupOptions: {
      output: {
        manualChunks(id) {
          if (!id.includes("node_modules")) return undefined;
          if (id.includes("echarts-gl") || id.includes("claygl")) return "echarts-gl";
          if (id.includes("echarts") || id.includes("zrender")) return "echarts-core";
          if (id.includes("@tanstack")) return "tanstack";
          if (id.includes("lucide-react")) return "icons";
          return "vendor";
        },
      },
    },
  },
  server: {
    allowedHosts: ["mini1.local", ".tail46d3fb.ts.net"],
    proxy: {
      "/api": {
        target: apiProxyTarget,
        changeOrigin: true,
      },
    },
  },
});
