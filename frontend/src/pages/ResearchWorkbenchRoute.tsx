  ...
  return <div className="space-y-5">
    ...
    {section === "overview" ? <>
      {!overview ? <p className="text-sm text-muted-foreground">Loading research state…</p> : <>
        ...
        <DiagnosticsSection diagnostics={(overview?.diagnostics ?? {}) as Record<string, any>} />
...
function DiagnosticsSection({ diagnostics }: { diagnostics: Record<string, any> }) {
  ...
  const rejectedCounts = rejected.counts ?? {};
  const efficiency = diagnostics.research_efficiency ?? {};
  const advisor = efficiency.advisor_responses ?? {};
  const drift = diagnostics.drift_concentration ?? {};
  const topSymbol = Array.isArray(drift.symbol_concentration) ? drift.symbol_concentration[0] : null;
  const components = Array.isArray(diagnostics.edge_waterfall?.components) ? diagnostics.edge_waterfall.components : [];
  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-2"><div><h2 className="text-lg font-semibold">Decision diagnostics</h2><p className="text-sm text-muted-foreground">Outcome comparisons and research efficiency stay separate from canonical paper P&amp;L.</p></div><StatusBadge tone={diagnostics.quality_status === "empty" ? "muted" : "info"}>{text(diagnostics.quality_status, "unknown")}</StatusBadge></div>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricTile label="Comparable outcomes" value={String(qualityCounts.comparable_counterfactual_count ?? 0)} caption="Selected return vs declared counterfactual." /><MetricTile label="Rejected setups" value={String(rejectedCounts.excluded_observations ?? 0)} caption={`${String(rejectedCounts.total_observations ?? 0)} persisted universe observations.`} /><MetricTile label="Valid forecasts" value={String(advisor.valid_resolved_forecast_count ?? 0)} caption="Resolved with valid evidence." /><MetricTile label="Top symbol share" value={topSymbol?.episode_share == null ? "—" : `${(Number(topSymbol.episode_share) * 100).toFixed(0)}%`} caption={topSymbol ? `${text(topSymbol.symbol)} of independent episodes.` : "No resolved episode slice."} /></div>
    <div className="grid gap-5 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Decision quality vs outcome</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p>Mean selected return: <strong>{formatPercent(quality.mean_selected_return)}</strong> · declared counterfactual: <strong>{formatPercent(quality.mean_stock_counterfactual_return)}</strong>.</p><p>Mean selection delta: <strong>{formatPercent(quality.mean_selection_delta)}</strong> · median: <strong>{formatPercent(quality.median_selection_delta)}</strong>.</p><p className="text-muted-foreground">{text(quality.methodology, "No methodology recorded.")}</p><p className="text-xs text-muted-foreground">Luck attribution is not inferred from one outcome. Explicit diagnosis: {String(qualityCounts.explicitly_diagnosed_count ?? 0)}.</p></CardContent></Card>
      <Card><CardHeader><CardTitle>Rejected opportunities</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p>Coverage: <strong>{rejected.coverage == null ? "—" : `${(Number(rejected.coverage) * 100).toFixed(0)}%`}</strong> · return observations: <strong>{String(rejectedCounts.excluded_with_return_observation ?? 0)}</strong> · executable outcomes: <strong>{String(rejectedCounts.excluded_with_executable_outcome ?? 0)}</strong>.</p><p className="text-muted-foreground">{text(rejected.methodology, "No rejection methodology recorded.")}</p>{Array.isArray(rejected.exclusion_reasons) && rejected.exclusion_reasons.length ? <div className="space-y-1 text-xs">{rejected.exclusion_reasons.slice(0, 4).map((row: Record<string, any>) => <div key={String(row.reason)} className="flex justify-between gap-3"><span>{text(row.reason)}</span><span>{String(row.count ?? 0)}</span></div>)}</div> : <p className="text-xs text-muted-foreground">No exclusion rows are available.</p>}</CardContent></Card>
    </div>
    <Card><CardHeader><CardTitle>Where edge is being lost</CardTitle></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">{components.map((component: Record<string, any>) => <div key={String(component.name)} className="rounded-md border border-border p-3 text-sm"><div className="font-medium">{String(component.name).replaceAll("_", " ")}</div><div className="mt-1 text-lg font-semibold">{component.value == null ? "Unavailable" : formatPercent(component.value)}</div><div className="mt-1 text-xs text-muted-foreground">{text(component.basis)}</div></div>)}</CardContent></Card>
    <Card><CardHeader><CardTitle>Research efficiency and concentration</CardTitle></CardHeader><CardContent className="grid gap-3 text-sm md:grid-cols-3"><div><div className="text-xs uppercase text-muted-foreground">Advisor responses</div><div className="mt-1 text-lg font-semibold">{String(advisor.response_count ?? 0)}</div><div className="text-xs text-muted-foreground">{String(advisor.succeeded_response_count ?? 0)} succeeded · {String(advisor.unpriced_cost_response_count ?? 0)} unpriced</div></div><div><div className="text-xs uppercase text-muted-foreground">Priced advisor cost</div><div className="mt-1 text-lg font-semibold">{advisor.priced_cost_usd == null ? "—" : `$${Number(advisor.priced_cost_usd).toFixed(2)}`}</div><div className="text-xs text-muted-foreground">Cost is not an investment-quality metric.</div></div><div><div className="text-xs uppercase text-muted-foreground">Regime evidence</div><div className="mt-1 text-lg font-semibold">{String(drift.independent_episode_count ?? 0)} episodes</div><div className="text-xs text-muted-foreground">{text(drift.methodology, "No slice methodology recorded.")}</div></div></CardContent></Card>
  </div>;
}
...
function PredictionsSection({ page, prompts, loadingMore, onLoadMore }: { page: ResearchClaimPage | null; prompts: ResearchPromptPage | null; loadingMore: boolean; onLoadMore: (cursor: string) => void }) {
  ...
  const quality: ForecastQualityView = page.quality ?? {};
  return <div className="space-y-5"><Card><CardHeader><CardTitle>Forecast quality</CardTitle></CardHeader><CardContent className="grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-5"><div><div className="text-xs uppercase text-muted-foreground">Valid resolved</div><div className="mt-1 text-lg font-semibold">{String(quality.valid_resolved_claims ?? 0)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Brier</div><div className="mt-1 text-lg font-semibold">{quality.brier_score == null ? "—" : Number(quality.brier_score).toFixed(3)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Directional</div><div className="mt-1 text-lg font-semibold">{formatPercent(quality.directional_accuracy)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Invalidation</div><div className="mt-1 text-lg font-semibold">{formatPercent(quality.invalidation_accuracy)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Coverage</div><div className="mt-1 text-lg font-semibold">{formatPercent(quality.coverage)}</div></div><p className="text-xs text-muted-foreground sm:col-span-2 xl:col-span-5">{text(quality.quality_score_basis, "Display metrics are unavailable until valid resolved claims mature.")}</p></CardContent></Card><PredictionQualityDetails quality={quality} total={Number(page.count?.total ?? 0)} /><Card><CardHeader><CardTitle>Calibration reliability</CardTitle></CardHeader><CardContent className="space-y-2 text-sm"><p className="text-muted-foreground">Observed rates use the declared event scorer; counts are shown per probability bin.</p>{Array.isArray(quality.calibration_bins) && quality.calibration_bins.length ? quality.calibration_bins.map((bin: Record<string, any>) => <div key={String(bin.bin)} className="flex items-center justify-between gap-3 border-b border-border py-2 text-xs last:border-0"><span>{Math.round(Number(bin.lower_bound) * 100)}–{Math.round(Number(bin.upper_bound) * 100)}% · n={String(bin.sample_count)}</span><span>pred {formatPercent(bin.mean_predicted)} · observed {formatPercent(bin.observed_rate)}</span></div>) : <p className="text-muted-foreground">No valid resolved events are available for calibration.</p>}<p className="mt-3 text-xs text-muted-foreground">Baseline rate: {formatPercent(quality.event_base_rate)} · skill versus baseline: {formatPercent(quality.skill_vs_base_rate)}</p></CardContent></Card><Card><CardHeader><CardTitle>Claim-level prediction ledger</CardTitle></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full text-left text-sm"><thead><tr className="border-b border-border text-xs uppercase text-muted-foreground"><th className="p-3">Issued</th><th className="p-3">Symbol / target</th><th className="p-3">Probability</th><th className="p-3">Horizon</th><th className="p-3">State</th><th className="p-3">Score</th></tr></thead><tbody>{page.rows.length ? page.rows.map((row) => <tr key={String(row.claim_id)} className="border-b border-border last:border-0"><td className="p-3"><Link className="underline-offset-4 hover:underline" to={`/research/predictions/${row.claim_id}?section=predictions`}>{date(row.issued_at)}</Link><div className="text-xs text-muted-foreground">cutoff {date(row.information_cutoff)}</div></td><td className="max-w-md p-3"><strong>{text(row.symbol)}</strong><div className="text-xs text-muted-foreground">{text(row.claim_kind)} · {text(row.statement)}</div></td><td className="p-3">{row.probability == null ? "—" : `${(Number(row.probability) * 100).toFixed(0)}%`}</td><td className="p-3">{text(row.horizon)}</td><td className="p-3"><StatusBadge tone={row.maturity_state === "resolved" ? "good" : row.maturity_state === "pending" || row.maturity_state === "waiting" ? "info" : "warn"}>{text(row.maturity_state, "pending")}</StatusBadge></td><td className="p-3">{row.calibration_error == null ? "—" : Number(row.calibration_error).toFixed(3)}</td></tr>) : <tr><td className="p-4 text-muted-foreground" colSpan={6}>No claims have been issued. Pending or unavailable data is not treated as prediction failure.</td></tr>}</tbody></table>{page.next_cursor ? <div className="border-t border-border p-3"><button type="button" className="rounded-md border border-border px-3 py-2 text-sm hover:bg-accent disabled:opacity-50" disabled={loadingMore} onClick={() => onLoadMore(page.next_cursor!)}>{loadingMore ? "Loading older claims…" : "Load older claims"}</button></div> : null}</CardContent></Card><Card><CardHeader><CardTitle>Prompt versions</CardTitle></CardHeader><CardContent className="space-y-2 text-sm">{prompts?.rows.length ? prompts.rows.map((row) => <div key={String(row.version)} className="flex flex-wrap items-center justify-between gap-2 border-b border-border py-2 last:border-0"><Link className="font-medium underline-offset-4 hover:underline" to={`/research/prompts/${encodeURIComponent(String(row.version))}?section=predictions`}>{String(row.version)}</Link><span className="text-muted-foreground">{text(row.semantic_effective_hash, "no hash").slice(0, 12)} · {text(row.claim_count, "0")} claims</span></div>) : <p className="text-muted-foreground">No prompt versions are persisted yet.</p>}</CardContent></Card></div>;
}

type ForecastQualityView = Partial<NonNullable<ResearchClaimPage["quality"]>> & { [key: string]: unknown };

function PredictionQualityDetails({ quality, total }: { quality: ForecastQualityView; total: number }) {
  const series = Array.isArray(quality.brier_time_series) ? quality.brier_time_series : [];
  const rate = (count: unknown) => total ? formatPercent(Number(count ?? 0) / total) : "—";
  return <div className="grid gap-5 lg:grid-cols-2">
    <Card><CardHeader><CardTitle>Outcome coverage</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3 text-sm"><div><div className="text-xs uppercase text-muted-foreground">Pending</div><div className="mt-1 font-semibold">{String(quality.pending_claims ?? 0)} · {formatPercent(quality.abstention_rate)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Unresolved</div><div className="mt-1 font-semibold">{String(quality.unresolved_claims ?? 0)} · {rate(quality.unresolved_claims)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Unsupported</div><div className="mt-1 font-semibold">{String(quality.unsupported_claims ?? 0)} · {rate(quality.unsupported_claims)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Invalid evidence</div><div className="mt-1 font-semibold">{String(quality.invalid_claims ?? 0)} · {rate(quality.invalid_claims)}</div></div><p className="col-span-2 text-xs text-muted-foreground">Pending claims await their declared observation clock. Unsupported and invalid records are excluded from numeric quality.</p></CardContent></Card>
    <Card><CardHeader><CardTitle>Run telemetry</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-3 text-sm"><div><div className="text-xs uppercase text-muted-foreground">Responses</div><div className="mt-1 font-semibold">{String(quality.response_count ?? 0)}</div></div><div><div className="text-xs uppercase text-muted-foreground">Mean latency</div><div className="mt-1 font-semibold">{quality.avg_latency_ms == null ? "—" : `${Math.round(Number(quality.avg_latency_ms))} ms`}</div></div><div><div className="text-xs uppercase text-muted-foreground">Tokens</div><div className="mt-1 font-semibold">{quality.input_tokens == null && quality.output_tokens == null ? "—" : `${String(quality.input_tokens ?? 0)} in · ${String(quality.output_tokens ?? 0)} out`}</div></div><div><div className="text-xs uppercase text-muted-foreground">Priced cost</div><div className="mt-1 font-semibold">{quality.cost_usd == null ? "—" : `$${Number(quality.cost_usd).toFixed(4)}`}</div></div><p className="col-span-2 text-xs text-muted-foreground">Cost status: {text(quality.cost_status, "unavailable")}; {String(quality.unpriced_response_count ?? 0)} response(s) have no priced provider cost.</p></CardContent></Card>
    {series.length ? <Card className="lg:col-span-2"><CardHeader><CardTitle>Brier score over resolution time</CardTitle></CardHeader><CardContent className="space-y-2 text-sm">{series.map((point: Record<string, any>) => <div key={String(point.resolved_at)} className="flex items-center justify-between border-b border-border py-2 last:border-0"><span>{date(point.resolved_at)} · n={String(point.sample_count)}</span><strong>{point.brier_score == null ? "—" : Number(point.brier_score).toFixed(3)}</strong></div>)}</CardContent></Card> : null}
  </div>;
}
