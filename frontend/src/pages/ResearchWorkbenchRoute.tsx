...
import { dateTime, humanize, money, numberValue, percent, shortDate, statusLabel, statusTone } from "@/presentation/labels";
...
export function ResearchWorkbenchRoute() {
  ...
  useEffect(() => {
    ...
    const request = section === "strategies"
      ? loadResearchStrategies(controller.signal).then(value => { if (!controller.signal.aborted) setStrategies(value); })
      : section === "predictions"
        ? Promise.all([loadResearchPredictions(controller.signal), loadResearchPrompts(controller.signal), loadLearningOverview(controller.signal)]).then(([claims, versions, state]) => { if (!controller.signal.aborted) { setPredictions(claims); setPrompts(versions); setOverview(state); } })
        : section === "experiments"
  ...
  return <div className="space-y-5">
    ...
    <WorkflowReadiness view="research" onDataChanged={refresh} />
    ...
    {section === "predictions" && (!error || predictions) ? <PredictionsSection page={predictions} prompts={prompts} lane={prediction} loadingMore={loadingMore} onLoadMore={(cursor) => loadMore("predictions", cursor)} /> : null}
...
function OverviewSection({ overview, strategy, prediction, events }: { overview: LearningOverviewPayload | null; strategy: Record<string, any>; prediction: Record<string, any>; events: Array<Record<string, any>> }) {
  ...
  return <div className="space-y-5"><section className="grid gap-4 xl:grid-cols-[1.2fr_1fr]"><StrategyPipeline lane={strategy} /><PromptPipeline lane={prediction} quality={predictionQuality} /></section><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><InsightMetric label="Funded paper fills" value={promptMetric(paper?.counts.filled_orders)} note={paper?.counts.filled_orders ? "Account fills, separate from research experiments" : "No funded fills; experiment activity is shown above"} /><InsightMetric label="Comparable cases" value={promptMetric((overview.diagnostics as any)?.decision_quality?.counts?.comparable_counterfactual_count)} note="Matched outcome evidence" /><InsightMetric label="Active strategy" value={humanize(strategy.deployed_version, "Not selected")} note="Deterministic policy remains in control" /><InsightMetric label="Forecast learning" value={predictionQuality.valid_resolved_claims ? String(predictionQuality.valid_resolved_claims) : prediction.status === "disabled" ? "Generation paused" : "Awaiting outcomes"} note="Independent resolved forecasts" /></section><NextStep strategy={strategy} prediction={prediction} /><DecisionDiagnostics diagnostics={(overview.diagnostics ?? {}) as Record<string, any>} /><Card><CardHeader><CardTitle>What changed</CardTitle><p className="text-sm text-muted-foreground">A short history of changes that can affect future decisions.</p></CardHeader><CardContent>{events.length ? <div className="space-y-2">{compactResearchEvents(events).slice(0, 12).map((event, index) => <div key={`${event.event_type}-${index}`} className="flex flex-wrap items-center justify-between gap-3 border-b border-border py-3 text-sm last:border-0"><span><strong>{eventLabel(event.event_type)}</strong><span className="ml-2 text-muted-foreground">{humanize(event.reason, "Evidence update")}{event.repeat_count > 1 ? ` · ${event.repeat_count} recorded updates (not independent outcomes)` : ""}</span></span><span className="text-muted-foreground">{date(event.event_at)}</span></div>)}</div> : <p className="text-sm text-muted-foreground">No strategy or prompt changes are recorded yet.</p>}</CardContent></Card></div>;
}

function StrategyPipeline({ lane }: { lane: Record<string, any> }) {
  const steps = strategySteps(lane);
  const blockers = laneBlockers(lane);
  return <Card className="overflow-hidden"><CardHeader className="border-b border-border bg-primary/[0.04]"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Strategy learning</p><CardTitle className="mt-1">{lifecycleHeadline(lane, "Strategy evidence collection")}</CardTitle></div><StatusBadge tone={strategyTone(lane.status)}>{strategyStatus(lane.status)}</StatusBadge></div></CardHeader><CardContent className="space-y-5 p-5"><CollectionProgress collection={lane.collection ?? {}} /><div className="grid gap-2 sm:grid-cols-4">{steps.map((step, index) => <div key={step.label} className="relative rounded-xl border border-border bg-background p-3">{index < steps.length - 1 ? <span className="absolute -right-2 top-1/2 hidden h-px w-4 bg-border sm:block" /> : null}<div className="flex items-center gap-2"><span className={`size-2.5 rounded-full ${step.state === "complete" ? "bg-emerald-600" : step.state === "current" ? "bg-amber-500" : "bg-muted-foreground/30"}`} /><span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{step.label}</span></div><p className="mt-3 text-sm font-semibold">{humanize(step.value)}</p><p className="mt-1 text-xs text-muted-foreground">{step.detail ?? "Current policy"}</p></div>)}</div>{lane.hypothesis ? <div className="rounded-lg border p-3 text-sm"><strong>Hypothesis: </strong>{String(lane.hypothesis)}{lane.falsification ? <p className="mt-2 text-muted-foreground"><strong>Falsification: </strong>{String(lane.falsification)}</p> : null}<p className="mt-2 text-xs text-muted-foreground">Evidence subject: revision {String(lane.subject_revision_id ?? "not recorded")} · {lane.challenger ? "challenger" : "incumbent"}</p></div> : null}<div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">{blockers.length ? blockers[0] : "All promotion checks still apply; job counts do not establish evidence quality."}</p><Link className="text-sm font-medium text-primary hover:underline" to="/research?section=strategies">Review strategy history →</Link></div></CardContent></Card>;
}

export function CollectionProgress({ collection }: { collection: Record<string, any> }) {
  const counts = collection.counts ?? {};
  if (!collection.management_status) return null;
  if (collection.management_status === "unavailable") return <section role="alert" className="rounded-lg border border-destructive/40 p-3 text-sm">
    <h3 className="font-semibold">Research collection status could not be read</h3>
    <p className="mt-1">Counts are not available; this is not an empty or healthy collection.</p>
    <Link className="mt-2 inline-block underline" to="/health">Inspect the research collection service</Link>
  </section>;
  return <section aria-label="Research collection progress" className="rounded-lg border border-border p-3 text-sm">
    <h3 className="font-semibold">{collection.management_status === "overdue" ? "Collection needs attention" : "Prospective research collection"}</h3>
    <p className="mt-1">{counts.entered ?? 0} open · {counts.pending ?? 0} awaiting entry · {counts.closed ?? 0} completed · {counts.unfilled ?? 0} unfilled · {counts.rejected ?? 0} rejected</p>
    <p className="mt-1 text-xs text-muted-foreground">{collection.basis}</p>
    {collection.last_checked_at ? <p className="mt-1 text-xs">Last manager check: {dateTime(collection.last_checked_at)} · {humanize(collection.management_status)}</p> : null}
    {collection.next_entry_deadline ? <p className="mt-1 text-xs">Next entry deadline: {dateTime(collection.next_entry_deadline)}</p> : null}
    <Link className="mt-2 inline-block text-primary underline" to="/portfolio/paper">Open experiment positions, P&amp;L and events →</Link>
  </section>;
}

export function PromptPipeline({ lane, quality }: { lane: Record<string, any>; quality: Record<string, any> }) {
...
function NextStep({ strategy, prediction }: { strategy: Record<string, any>; prediction: Record<string, any> }) {
  ...
  return <section className="rounded-xl border border-amber-500/30 bg-amber-500/[0.07] p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-amber-800 dark:text-amber-200">Next decision</p><h2 className="mt-1 text-base font-semibold">{strategy.status === "misconfigured" ? "Repair strategy parameters / implementation binding" : blockers.length ? blockers[0] : "Continue collecting matched evidence"}</h2><p className="mt-1 text-sm text-muted-foreground">Learning stays advisory until its statistical, safety, and forward checks are complete.</p></div><Link className="rounded-md border border-amber-500/40 px-3 py-2 text-sm font-medium hover:bg-amber-500/10" to={strategy.status === "misconfigured" ? "/research?section=strategies" : blockers.length && String(strategy.status).includes("paper") ? "/portfolio/paper" : "/research?section=experiments"}>{blockers.length ? "Inspect blocker" : "Review experiments"}</Link></div></section>;
}
...
function DecisionDiagnostics({ diagnostics }: { diagnostics: Record<string, any> }) {
  ...
  return <section><div className="mb-3"><h2 className="text-xl font-semibold">Decision quality</h2><p className="text-sm text-muted-foreground">Outcome evidence is separate from paper P&L and never treated as a trading result by itself.</p></div><div className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle className="text-base">What the evidence says</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-3"><MiniStat label="Comparable cases" value={promptMetric(counts.comparable_counterfactual_count)} /><MiniStat label="Selection edge" value={percent(quality.mean_selection_delta)} /><MiniStat label="Rejected setups" value={promptMetric(rejected.counts?.excluded_observations)} /></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Where evidence is missing</CardTitle></CardHeader><CardContent className="space-y-2">{components.length ? components.slice(0, 4).map((component: Record<string, any>) => <div key={String(component.name)} className="flex items-center justify-between gap-3 border-b border-border py-2 text-sm last:border-0"><span>{humanize(component.name)}</span><span className="text-muted-foreground">{component.value == null ? "Not available" : percent(component.value)}</span></div>) : <p className="text-sm text-muted-foreground">No edge-loss diagnosis is available yet.</p>}</CardContent></Card></div></section>;
}
...
function PredictionsSection({ page, prompts, lane, loadingMore, onLoadMore }: { lane: Record<string, any>; page: ResearchClaimPage | null; prompts: ResearchPromptPage | null; loadingMore: boolean; onLoadMore: (cursor: string) => void }) {
  if (!page) return <ResearchSkeleton />;
  const quality = (page.quality ?? {}) as Record<string, any>;
  const progress = predictionProgress(quality);
  return <div className="space-y-5"><ForecastCollection lane={lane} quality={quality} /><section className="rounded-xl border border-border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Forecast quality</p><h2 className="mt-1 text-2xl font-semibold">Are the probabilities calibrated?</h2><p className="mt-1 text-sm text-muted-foreground">Only independently resolved and evidence-valid claims contribute to the curves below.</p></div><StatusBadge tone={quality.status === "insufficient_evidence" ? "warn" : "info"}>{statusLabel(quality.status, "Collecting evidence")}</StatusBadge></div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><InsightMetric label="Resolved forecasts" value={promptMetric(quality.valid_resolved_claims)} note={progress.detail} />{quality.brier_score != null ? <InsightMetric label="Brier score" value={promptMetric(quality.brier_score, "brier")} note="Lower is better" /> : null}{quality.directional_accuracy != null ? <InsightMetric label="Directional accuracy" value={promptMetric(quality.directional_accuracy, "percent")} note="Resolved directional claims" /> : null}{quality.coverage != null ? <InsightMetric label="Coverage" value={promptMetric(quality.coverage, "percent")} note="Issued claims with outcomes" /> : null}<InsightMetric label="Next gate" value={progress.state === "unavailable" ? "Not established" : progress.remaining ? `${progress.remaining} more` : "Count floor met"} note="Independent outcomes" /></div></section><CalibrationVisual quality={quality} /><BrierVisual quality={quality} /><PromptComparison prompts={prompts} /><ClaimLedger page={page} loadingMore={loadingMore} onLoadMore={onLoadMore} /></div>;
}
...
function BrierVisual({ quality }: { quality: Record<string, any> }) {
  const series = Array.isArray(quality.brier_time_series) ? quality.brier_time_series.filter((point: Record<string, any>) => typeof point.brier_score === "number" && Number.isFinite(point.brier_score) && point.brier_score >= 0 && point.brier_score <= 1 && Number.isFinite(Date.parse(String(point.resolved_at)))).sort((a: Record<string, any>, b: Record<string, any>) => Date.parse(a.resolved_at) - Date.parse(b.resolved_at)) : [];
...
function PromptComparison({ prompts }: { prompts: ResearchPromptPage | null }) {
  ...
  if (!active) return <EmptyVisual title="Active prompt is not established" detail="Prompt history exists, but none is marked as the active baseline. History order does not establish deployment authority." />;
  const challenger = rows.find((row) => row.version !== active.version && row.parent_version === active.version);
...
export function compactResearchEvents(events: Array<Record<string, any>>): Array<Record<string, any>> {
  const seen = new Map<string, Record<string, any>>();
  for (const event of events) {
    const key = event.event_type === "strategy_evaluation" ? `${event.event_type}:${event.original_version}:${event.reason}` : `${event.event_type}:${event.new_version}:${event.event_at}`;
    const existing = seen.get(key);
    if (existing) existing.repeat_count += 1;
    else seen.set(key, { ...event, repeat_count: 1 });
  }
  return [...seen.values()];
}


export function ForecastCollection({ lane, quality }: { lane: Record<string, any>; quality: Record<string, any> }) {
  return <section className="rounded-lg border border-border bg-card p-4 text-sm" aria-label="Forecast collection lifecycle">
    <h2 className="font-semibold">{lane.generation_enabled === false ? "New forecast generation is paused" : "Forecast collection and settlement"}</h2>
    {lane.pause_reason ? <p className="mt-2">{lane.pause_reason}</p> : null}
    <p className="mt-2">{promptMetric(quality.total_claims ?? quality.claim_count)} issued · {promptMetric(quality.pending_claims)} pending · {promptMetric(quality.valid_resolved_claims)} independently resolved</p>
    <p className="mt-2 text-xs text-muted-foreground">Unresolved, unsupported and dependent claims are not scored as losses or counted as independent evidence. {lane.settlement_enabled === false ? "Settlement is disabled; recorded claims will not progress until it is resumed." : "The settlement worker resolves recorded claims after their observation window."}</p>
    {lane.generation_enabled === false || lane.settlement_enabled === false ? <a className="mt-2 inline-block underline" href="/agent">Forecast controls</a> : null}
  </section>;
}
