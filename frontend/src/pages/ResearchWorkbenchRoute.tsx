...
export function ResearchWorkbenchRoute() {
  ...
  useEffect(() => {
    ...
    const request = section === "strategies"
      ...
      : section === "predictions"
        ? Promise.all([loadResearchPredictions(controller.signal), loadResearchPrompts(controller.signal)]).then(([claims, versions]) => { if (!controller.signal.aborted) { setPredictions(claims); setPrompts(versions); } })
        : section === "experiments"
...
function OverviewSection({ overview, strategy, prediction, events }: { overview: LearningOverviewPayload | null; strategy: Record<string, any>; prediction: Record<string, any>; events: Array<Record<string, any>> }) {
  ...
  return <div className="space-y-5"><section className="grid gap-4 xl:grid-cols-[1.2fr_1fr]"><StrategyPipeline lane={strategy} /><PromptPipeline lane={prediction} quality={predictionQuality} /></section><section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><InsightMetric label="Paper fills" value={promptMetric(paper?.counts.filled_orders)} note={paper?.counts.filled_orders ? "Available for performance learning" : "Waiting for the first paper-ready fill"} /><InsightMetric label="Comparable cases" value={promptMetric((overview.diagnostics as any)?.decision_quality?.counts?.comparable_counterfactual_count)} note="Matched outcome evidence" /><InsightMetric label="Active strategy" value={humanize(strategy.deployed_version, "Not selected")} note="Deterministic policy remains in control" /><InsightMetric label="Forecast learning" value={predictionQuality.valid_resolved_claims ? String(predictionQuality.valid_resolved_claims) : "Not started"} note="Independent resolved forecasts" /></section><NextStep strategy={strategy} prediction={prediction} /><DecisionDiagnostics diagnostics={(overview.diagnostics ?? {}) as Record<string, any>} /><Card><CardHeader><CardTitle>What changed</CardTitle><p className="text-sm text-muted-foreground">A short history of changes that can affect future decisions.</p></CardHeader><CardContent>{events.length ? <div className="space-y-2">{compactResearchEvents(events).slice(0, 12).map((event, index) => <div key={`${event.event_type}-${index}`} className="flex flex-wrap items-center justify-between gap-3 border-b border-border py-3 text-sm last:border-0"><span><strong>{eventLabel(event.event_type)}</strong><span className="ml-2 text-muted-foreground">{humanize(event.reason, "Evidence update")}{event.repeat_count > 1 ? ` · ${event.repeat_count} recorded updates (not independent outcomes)` : ""}</span></span><span className="text-muted-foreground">{date(event.event_at)}</span></div>)}</div> : <p className="text-sm text-muted-foreground">No strategy or prompt changes are recorded yet.</p>}</CardContent></Card></div>;
}
