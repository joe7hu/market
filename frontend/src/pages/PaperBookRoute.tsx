...
import { PaperPerformancePanel } from "@/components/market/PaperPerformancePanel";
...
import { humanize, lifecycleLabel, money, percent, statusLabel, statusTone } from "@/presentation/labels";
...
export function PaperBookRoute() {
  ...
  useEffect(() => {
    ...
    // A same-scope poll must not unmount the chart or discard its zoom. A
    // filter change still clears the old scope before a new packet arrives.
    if (!sameScope) {
      setPerformance(null);
      setTrades([]);
      setNextCursor(null);
  ...
  const loadOlder = () => {
    if (!nextCursor || paging || loading || refreshing || loadedScopeKey !== scopeKey) return;
  ...
  const onRangeChange = (from: string, to: string) => updateFilters({ date_from: from, date_to: to, period: "custom" });
  ...
  return <div className="space-y-5">
    <PageHeader eyebrow="Portfolio · Paper" title="Paper portfolio" subtitle="Account equity, verified P&L, and trade activity." actions={<div className="flex gap-2"><button className="rounded-md border border-border px-3 py-2 text-sm" disabled={loading || refreshing || paging} onClick={() => setReload(value => value + 1)}>{refreshing ? "Refreshing…" : "Reload book"}</button><a className="rounded-md border border-border px-3 py-2 text-sm underline-offset-4 hover:underline" href={paperTradesExportUrl(filters)} download>Export CSV</a></div>} />
    ...
    {error ? <div role="alert" className="rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">{visiblePerformance ? "Retained paper snapshot may be stale. " : ""}{error}</div> : null}
    ...
    {!scopeLoading && visiblePerformance ? <>
      {account && account.status !== "unfunded" ? <section className="rounded-2xl border border-border bg-card p-5 sm:p-7" aria-label="Funded paper account">
        ...
        <details className="mt-3 text-xs text-muted-foreground"><summary className="cursor-pointer">Funding and balance evidence</summary><p className="mt-2">{account.authorization}</p><p>Recorded {account.opened_at ? new Date(account.opened_at).toLocaleString() : "—"}. Balances as of {account.as_of ? new Date(account.as_of).toLocaleString() : "—"}.</p><p>Whole-account balances do not change with trade filters. No live brokerage orders.</p></details>
      ...
      <section className="rounded-2xl border border-border bg-card p-5 shadow-sm sm:p-7" aria-label="Paper portfolio summary">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Realized P&L · {period === "custom" ? "Selected range" : period.toUpperCase()}</p><h2 className="mt-2 text-3xl font-semibold tracking-tight sm:text-5xl">{hasVerifiedPerformance ? money(visiblePerformance.realized_pnl, true) : "No verified exits yet"}</h2><p className="mt-2 text-sm text-muted-foreground">{hasVerifiedPerformance ? "Verified, fill-backed exits in this scope." : "Entries and open positions appear in trade activity below."}</p></div><StatusBadge tone={learningNotStarted ? "muted" : statusTone(visiblePerformance.quality_status)}>{learningNotStarted ? "Awaiting fills" : statusLabel(visiblePerformance.quality_status)}</StatusBadge></div>
      ...
      <PaperPerformancePanel performance={visiblePerformance} chart={chart} points={points} drawdownPoints={drawdownPoints} onChart={(value) => updateFilters({ chart: value })} onSelect={(tradeId) => navigate(`/portfolio/paper/trades/${encodeURIComponent(tradeId)}${location.search}`)} onRangeChange={onRangeChange} />
      {hasVerifiedPerformance ? <details className="rounded-xl border border-border bg-card p-4"><summary className="cursor-pointer text-sm font-semibold">Performance attribution</summary><div className="mt-4"><Attribution performance={visiblePerformance} /></div></details> : null}
      <TradeBlotter trades={trades} performance={visiblePerformance} locationSearch={location.search} onLoadOlder={loadOlder} paging={paging} refreshing={refreshing} hasNext={Boolean(nextCursor)} />
    </> : null}
    <PaperObservations />
  </div>;
}
...
function LearningLifecycle({ performance }: { performance: PaperPerformance }) {
  ...
  return <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-dashed p-4 text-sm"><div><p className="font-medium">No funded paper fills in this scope</p><p className="mt-1 text-xs text-muted-foreground">{staged} unfilled orders · {evidenceReason(performance.missing_evidence_reasons?.[0] ?? "no_paper_fills")}</p></div><Link className="font-medium text-primary hover:underline" to="/opportunities">Qualified opportunities →</Link></div>;
}
