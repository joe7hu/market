...
import { paperValuationDescription } from "@/presentation/paperProgress";
...
export function PaperTradeRoute() {
  ...
  return <div className="space-y-5">
    <button className="float-right rounded border px-3 py-1 text-sm" onClick={() => setReload(value => value + 1)}>Reload trade</button>
...
function buildTimeline(trade: PaperTrade): TimelineEvent[] {
  ...
  if (trade.mark_status === "verified" && !trade.mark_stale && trade.mark_observed_at && trade.remaining_quantity) events.push({ label: trade.mark?.session_state === "last_completed_session" ? "Last-session valuation (not a fill)" : "Verified valuation (not a fill)", at: trade.mark_observed_at, kind: "mark", price: trade.mark_price });
  const outcome = safeRecord(trade.outcome);
...
function MiniTradeCharts({ trade, timeline }: { trade: PaperTrade; timeline: TimelineEvent[] }) {
  const priceEvents = timeline.filter((event) => event.price != null && Number.isFinite(event.price) && event.at && Number.isFinite(Date.parse(event.at)));
  const values = priceEvents.map((event) => Number(event.price));
  if (!values.length) return null;
  const min = Math.min(...values);
  const max = Math.max(...values);
  return <Card><CardHeader><CardTitle>Holding path</CardTitle><p className="text-sm text-muted-foreground">Recorded fills and the latest verified mark only—not a continuous holding-price history. Points are not joined across missing observations.</p></CardHeader><CardContent><div className="grid gap-4 md:grid-cols-2"><div><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Observed price</p><svg viewBox="0 0 600 220" role="img" aria-label="Observed trade price path" className="h-48 w-full rounded-lg bg-muted/30">{values.map((value, index) => <circle key={`${value}-${index}`} cx={20 + index * (560 / Math.max(1, values.length - 1))} cy={190 - ((value - min) / Math.max(0.01, max - min)) * 150} r="6" fill="#2563eb"><title>{money(value)}</title></circle>)}</svg></div><div className="flex items-center justify-center rounded-lg border border-dashed border-border p-5 text-center"><div><p className="font-medium">P&L attribution</p><p className="mt-1 text-sm text-muted-foreground">{trade.realized_pnl == null ? "Unavailable until a verified exit is recorded." : `Verified realized result: ${money(trade.realized_pnl, true)}`}</p></div></div></div></CardContent></Card>;
}
