...
export function PaperTradeRoute() {
  ...
  useEffect(() => {
    ...
    void loadPaperTrade(tradeId, controller.signal).then(setTrade).catch((reason) => { if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : "Trade unavailable."); });
    ...
    <div className="space-y-5">
      ...
      {trade ? <>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Card><CardHeader><CardTitle>Execution</CardTitle></CardHeader><CardContent className="space-y-3 text-sm"><div className="grid grid-cols-2 gap-3"><div><span className="text-muted-foreground">Lifecycle</span><p className="font-medium">{trade.lifecycle}</p></div><div><span className="text-muted-foreground">Paper only</span><p className="font-medium">{execution.paper_only ? "Yes" : "Blocked"}</p></div><div><span className="text-muted-foreground">Filled quantity</span><p className="font-medium">{String(trade.filled_quantity ?? 0)}</p></div><div><span className="text-muted-foreground">Remaining</span><p className="font-medium">{String(trade.remaining_quantity ?? 0)}</p></div><div><span className="text-muted-foreground">Entry</span><p className="font-medium">{money(trade.entry_price)}</p></div><div><span className="text-muted-foreground">Staged limit</span><p className="font-medium">{money(trade.staged_limit_price)}</p></div></div><p className="text-xs text-muted-foreground">A staged limit is not a fill. Multiplier: {execution.multiplier_evidence ?? "unavailable"}; current mark: {trade.mark_status ?? "unavailable"}.</p>{fills.length ? <Table><TableHeader><TableRow><TableHead>At</TableHead><TableHead>Action</TableHead><TableHead>Quantity</TableHead><TableHead>Price</TableHead></TableRow></TableHeader><TableBody>{fills.map((fill) => <TableRow key={String(fill.id)}><TableCell>{date(fill.created_at)}</TableCell><TableCell>{String(fill.action ?? "")}</TableCell><TableCell>{String(fill.quantity ?? "")}</TableCell><TableCell>{money(typeof fill.price === "number" ? fill.price : null)}</TableCell></TableRow>)}</TableBody></Table> : <p className="rounded-md bg-muted p-3 text-xs">No deterministic fill journal. No paper position or P&L is claimed.</p>}</CardContent></Card>
