...
export function workerStateLabel(worker: Record<string, unknown>): string {
  return worker.status === "skipped" && String(object(worker.summary).reason ?? "").startsWith("no_") ? "No eligible work" : humanize(worker.status);
}
export function operationCount(population: unknown, statuses: string[]): string {
  const record = object(population);
  if (record.status !== "available" && record.status !== "partial") return "Not available";
...
export function observationScope(population: unknown): string {
  return object(population).status === "partial" ? "partial population" : "full population";
}
...
function stateClass(state: unknown): string {
  return ["failed", "overdue", "unavailable"].includes(String(state)) ? "text-destructive" :
    ["available", "succeeded", "running"].includes(String(state)) ? "text-primary" :
  ...
  return <section className="rounded-xl border border-border bg-card p-4" aria-label="Workflow readiness">
    ...
    {snapshot ? <>
      ...
      {view !== "market" && view !== "research" ? <div className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">Research observations · open / closed</p><p className="mt-1 font-semibold">{operationCount(observations, ["entered"])} / {operationCount(observations, ["closed"])}</p><p className="mt-1 text-xs text-muted-foreground">{observations.status === "partial" ? "Partial coverage; " : ""}Separate from funded paper-account P&amp;L</p></div>
      ...
      {view === "paper" || view === "health" ? <details className="mt-4" open={view === "paper"}><summary className="cursor-pointer text-sm font-medium">Execution funnel and unresolved reasons</summary><div className="mt-3 grid gap-4 lg:grid-cols-2">
        <div><p className="mb-2 text-xs text-muted-foreground">Recorded research observations, {observationScope(observations)}</p>{observations.status !== "available" && observations.status !== "partial" ? <p>Observation counts could not be read.</p> : reasons.length ? reasons.slice(0, 8).map((item, i) => <div key={i} className="flex items-start justify-between gap-3 border-b py-2 text-xs"><span><strong>{humanize(item.status)}</strong> · {humanize(item.reason, "No reason recorded")}</span><span className="tabular-nums">{count(item.count)}</span></div>) : <p className="text-sm">No prospective observations have been recorded.</p>}</div>
