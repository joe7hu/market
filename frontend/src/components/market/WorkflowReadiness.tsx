...
const WORKER_NAMES: Record<string, string> = {
  update_market_data: "Market data", update_market_valuations: "Valuations", refresh_market_publication: "Market publication",
...
export function WorkflowReadiness({ view = "today", onDataChanged }: { view?: ReadinessView; onDataChanged?: () => void }) {
  ...
  const workers = snapshot?.workers?.filter(worker => JOBS[view].includes(worker.job)) ?? [];
  const blockers = snapshot?.blockers?.filter(item => view === "today" || view === "health" ||
  ...
  return <section className="rounded-xl border border-border bg-card p-4" aria-label="Workflow readiness">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold">{view === "paper" ? "Paper execution — what is happening now" : view === "research" ? "Evidence collection health" : "Workflow readiness"}</h2>
      <p className="mt-1 text-xs text-muted-foreground">{snapshot ? `Recorded ${dateTime(snapshot.as_of)} · ${snapshot.market_session === "regular" ? "Regular market session" : `Market closed · next session ${dateTime(snapshot.next_session_at)}`}` : "Loading recorded workflow state…"}</p></div>
    ...
    {snapshot ? <>
      {view !== "market" && view !== "research" ? <div className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">Funded paper book · open / exited orders</p><p className="mt-1 font-semibold">{operationCount(paper, ["entered", "partial_exited"])} / {operationCount(paper, ["exited"])}</p><p className="mt-1 text-xs text-muted-foreground">{paper.entries_enabled === true ? "Options entry master switch enabled; lane, qualification and risk gates still apply" : "Options entry master switch disabled; existing risk is still managed"}</p></div>
      ...
      {blockers.length ? <div className="mt-3 space-y-2">{blockers.slice(0, 3).map((item, i) => <div key={`${item.capability}-${i}`} className="rounded-md border border-amber-300/60 p-3 text-sm"><strong>{item.capability}: </strong>{humanize(item.reason)}<p className="mt-1 text-xs text-muted-foreground">{item.action} <Link className="text-primary underline" to={item.href}>Review →</Link></p></div>)}</div> : null}
