...
export function FeedPage({ data, lastRefresh, loading, onRefresh, onOpenTicker }: { data: PanelData; model: AppModel; lastRefresh: Date | null; loading: boolean; onRefresh: () => void; onOpenTicker: OpenTicker }) {
  ...
  const queue = rows(data.decisionQueue);
  const readiness = rows(data.decisionReadiness);
  return (
    <WorkspacePage
      eyebrow="Signal feed"
      title="Feed"
      ...
      metrics={[
        ...
        ["Decision Queue", queue.length, "ranked backend rows", queue.length ? "info" : "muted"],
        ["Ready", readiness.filter((row) => textField(row, ["status"]) === "ready").length, "decision-readiness rows", "good"],
      ]}
    >
      <RowsSection title="Signal Feed" rows={feed.length ? feed : queue} onOpenTicker={onOpenTicker} />
      <RowsSection title="Decision Readiness" rows={readiness} onOpenTicker={onOpenTicker} />
    </WorkspacePage>
  );
}
...
function DossierCard({ title, row }: { title: string; row: RowRecord }) {
  ...
  const status = displayField(row, ["status", "decision", "grade"], "Loaded");
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      <p className="text-sm leading-6">{fullField(row, ["summary", "thesis", "reason", "decision_basis"], "No summary")}</p>
