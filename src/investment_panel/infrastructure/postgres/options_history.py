...
from investment_panel.infrastructure.postgres.options_history_stale import defer_stale_running_captures
from investment_panel.infrastructure.postgres.options_history_terminal import defer_capture as _defer_capture
from investment_panel.infrastructure.postgres.options_history_terminal import fail_capture as _fail_capture
from investment_panel.infrastructure.postgres.options_history_v3 import OptionHistoryV3Materializer
from investment_panel.infrastructure.postgres.options_history_health import history_health
from investment_panel.infrastructure.postgres.options_history_capture import store_capture as _store_capture
from investment_panel.infrastructure.postgres.options_history_policy import OptionHistoryPolicyRepository, PolicyConflict
...
def _surface_grid_payload(
    *, symbol: str, snapshot_id: int, rows: Sequence[dict[str, Any]], option_types: Sequence[str]
) -> dict[str, Any]:
    """Build the bounded grid directly beside the history query it sequences."""
    ...
    for kind in option_types:
        ...
        surfaces[kind] = [
            [_interpolate_surface(
                [(x, iv) for (dte, x), iv in grouped.items() if dte == y], point
    ...
    return {
        "snapshot_id": snapshot_id, "symbol": symbol.upper(), "x": x_values,
        "y": y_values, "surfaces": surfaces, "observed": observed,
    }


def _interpolate_surface(points: Sequence[tuple[float, Any]], x: float) -> float | None:
...
class OptionHistoryRepository:
    ...
    def surface_grid(
        ...
        *,
        ...
        return _surface_grid_payload(symbol=symbol, snapshot_id=snapshot_id, rows=rows, option_types=[option_type])
