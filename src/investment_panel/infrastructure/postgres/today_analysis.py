...
from investment_panel.infrastructure.postgres.analysis import AnalysisRepository, current_option_publication_rows
from investment_panel.infrastructure.postgres.agent_telemetry import AgentTelemetryRepository
...
from investment_panel.domain.options.preopen_forecast import backtest_qqq_preopen_model, evaluate_qqq_forecast, qqq_preopen_forecast
...
def refresh_today_publication(
    ...
    preopen = [{
        ...
        "headline": (narrative or {}).get("headline") or "Portfolio and research review",
        "summary": (narrative or {}).get("narrative") or f"{len(holdings)} holdings, {len(option_items)} option assessments, {len(review_rows)} thesis reviews, {len(source_items)} source changes, and {len(catalyst_rows)} near-term catalysts.",
