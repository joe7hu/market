...
def portfolio_rows(config: AppConfig, *, connection: Any | None = None) -> list[dict[str, Any]]:
    ...
    rows = connection.execute(
        ...
        SELECT p.symbol, p.name, p.asset_class, p.sector, p.industry, p.category,
               ...
               q.price, q.change_pct, q.change_abs, q.currency,
               q.observed_at AS quote_observed_at,
               q.available_at AS quote_available_at,
               q.source_id AS quote_source, q.source_kind AS quote_source_kind,
               q.trading_date AS quote_trading_date, q.valuation_status
        FROM positions p
    ...
    for source in rows:
        ...
        valuation_available = valuation_price is not None and row["avg_cost"] is not None
        row["valuation_available"] = valuation_available
        row["currency"] = str(row.get("currency") or "USD")
        if row.get("change_abs") is not None:
            row["day_change"] = float(row["change_abs"]) * row["quantity"]
        if row.get("change_pct") is not None:
            row["day_change_pct"] = float(row["change_pct"])
