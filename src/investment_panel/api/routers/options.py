...
from investment_panel.api.options_history_contracts import (
    IVSurfaceGrid,
    IVCurveSet,
    OptionAnomalyPage,
    OptionChainPage,
...
from investment_panel.api.routers.options_research import router as research_router
from investment_panel.settings import AppConfig
from investment_panel.domain.decision import ExpressionKind, TickerDecision
...
def recovery_option_health(actions: OptionsActions = Depends(dependencies.get_options_actions)) -> dict[str, Any]:
    """Recovery capacity, lease, storage, and scheduler diagnostics for Health only."""
    return actions.recovery_health()
...
def recovery_events(
    ...
    return actions.recovery_events(
...
def options_workspace(
    ...
    return actions.workspace(symbol=symbol, lane=lane)
...
def verify_static_arbitrage_candidate(
    candidate_id: int,
    actions: OptionsActions = Depends(dependencies.get_options_actions),
    _request=Depends(dependencies.get_authorized_request),
) -> dict[str, Any]:
    try:
        return actions.verify_static_arbitrage(candidate_id)
    except ValueError as exc:
...
def submit_agent_thesis(
    ...
    try:
        result = actions.submit_thesis(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    panel_snapshot.invalidate_context_cache()
    return result
...
def submit_agent_postmortem(
    ...
    try:
        result = actions.submit_postmortem(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    panel_snapshot.invalidate_context_cache()
    return result
...
def promote_strategy_mutation_endpoint(
    ...
    try:
        result = actions.promote_strategy(proposal_id, approved_by=approved_by)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    panel_snapshot.invalidate_context_cache()
    return result
