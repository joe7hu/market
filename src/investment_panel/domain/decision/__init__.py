"""Explicit exports for decision calendar, freshness, and brief rules."""

from investment_panel.domain.decision.brief import (
    GATE_LABELS,
    brief_summary,
    is_no_trade_action,
    ticker_decision_brief,
)
from investment_panel.domain.decision.brief_options import is_option_expired
from investment_panel.domain.decision.calendar import (
    classify_freshness,
    completed_trading_dates,
    easter_date,
    is_market_open,
    is_us_equity_early_close,
    is_us_market_day,
    last_weekday,
    latest_completed_market_day,
    market_deadline_passed,
    valuation_mark_is_stale,
    market_session_bounds,
    market_session_elapsed,
    normalized_utc,
    nth_weekday,
    observed_fixed_holiday,
    trading_day_lag,
    us_market_holidays,
)
from investment_panel.domain.decision.coerce import (
    decode,
    dedupe_freshness,
    latest_by_symbol,
    recency_points,
    related_symbols,
)
from investment_panel.domain.decision.brief_coerce import jsonable
from investment_panel.domain.decision.constants import (
    ARCO_STALE_DAYS,
    DAILY_ANALYSIS_SOURCES,
    DAILY_STALE_DAYS,
    FILING_STALE_DAYS,
    FRESHNESS_ORDER,
    INTRADAY_STALE_HOURS,
    MARKET_CLOSE,
    MARKET_OPEN,
    MARKET_TZ,
    PRIMARY_EVIDENCE_SOURCES,
    STATIC_SOURCES,
    SYMBOL_RE,
)
from investment_panel.domain.decision.freshness import (
    best_freshness,
    default_freshness_detail,
    eligibility_detail,
    overall_decision_freshness,
    stale_after_label,
    symbol_freshness_detail,
    top_source_cluster,
    worst_freshness,
)
from investment_panel.domain.decision.grading import (
    action_grade_for,
    apply_blocking_penalties,
    catalyst_window,
    decision_basis,
    gate_reasons,
    invalidation_for,
    portfolio_impact,
)
from investment_panel.domain.decision.readiness import (
    has_required_valuation_context,
    readiness_blockers,
    readiness_missing_inputs,
    readiness_next_action,
    readiness_portfolio_fit,
    readiness_status,
)
from investment_panel.domain.decision.resolution import (
    AuthorizationMode,
    DataQuality,
    DecisionResolutionV2,
    RESOLUTION_CONTRACT_VERSION,
    ResolutionAction,
    ResolutionEligibility,
    ResolutionLifecycle,
    ResolutionProvenance,
    build_decision_resolution,
    next_action_for,
    resolution_from_legacy,
    resolution_from_historical,
    resolution_from_published,
)

# Phase 2 is an additive contract layer. It does not replace the canonical
# MarketStateSnapshot or feed posterior values into ticker ranking.
from investment_panel.domain.market.phase2 import (  # noqa: E402
    CoverageVector,
    CoverageVectorRow,
    DimensionPosterior,
    EventObservation,
    MarketStatePosterior,
    PITObservation,
    Phase2Source,
    Phase2Status,
    ScenarioPath,
    assess_crypto_venue_data,
    assess_option_oi_volume_sla,
    build_coverage_vector,
    build_market_state_posterior,
    build_scenario_paths,
    parse_coinmetrics_derivatives,
    parse_corporate_expectations,
    parse_event_consensus,
    parse_fred_alfred,
    parse_treasury_yield_curve,
    posterior_can_influence_rank,
    replay_scenario_path,
    select_point_in_time,
    source_status,
)
from investment_panel.domain.decision.ticker import (
    opportunity_rank_blocker,
    trade_plan_rank_identity_matches,
    CONTRACT_VERSION as TICKER_DECISION_CONTRACT_VERSION,
    MARKET_DIMENSIONS,
    MARKET_HORIZONS,
    MARKET_SOURCE_PRIORITY,
    MARKET_REQUIRED_DIMENSIONS,
    MARKET_REQUIRED_DIMENSIONS_BY_HORIZON,
    OPPORTUNITY_EPISODE_CONTRACT_VERSION,
    TRADE_EXPRESSION_CONTRACT_VERSION,
    TRADE_PLAN_CONTRACT_VERSION,
    OUTCOME_ATTRIBUTION_CONTRACT_VERSION,
    OUTCOME_ATTRIBUTION_EVALUATION_VERSION,
    CapitalAction,
    CapitalActionType,
    capital_action_from_resolution,
    CoverageMatrix,
    CoverageMatrixRow,
    DataRequest,
    EvidenceItem,
    EvidencePolarity,
    ExpressionDecision,
    ExpressionKind,
    Horizon,
    HorizonDecision,
    InputManifest,
    InputLineage,
    Invalidation,
    MarketDimensionState,
    MarketEvidenceAssessment,
    MarketStateSnapshot,
    NumericRange,
    OpportunityEpisodeStatus,
    PriceRange,
    RiskPolicy,
    PortfolioImpact,
    ScenarioOutcome,
    SignalDeclaration,
    SignalEvidenceState,
    Stance,
    OpportunityEpisode,
    TradeExpression,
    TradePlan,
    market_evidence_for_decision,
    market_required_dimensions,
    OutcomeAttribution,
    OutcomeAttributionState,
    OutcomeEvidence,
    OutcomeEvidenceState,
    PaperExecutionOutcome,
    TickerDecision,
    apply_opportunity_rank_safety,
    bind_trade_plan,
    build_opportunity_episode,
    build_trade_plan,
    build_ticker_decision,
    expression_decision_from_trade_expression,
    expression_decision_to_trade_expression,
    opportunity_episode_from_legacy,
    opportunity_episode_id,
    trade_expression_from_expression_decision,
    trade_expression_from_legacy,
    trade_expression_identity,
    outcome_attribution_id,
    outcome_attribution_stable_key,
    portfolio_impact_from_persisted,
    portfolio_impact_id_for_persisted,
    portfolio_impacts_from_persisted,
    trade_plan_from_persisted,
)
from investment_panel.domain.decision.ticker_learning import (
    TICKER_LEARNING_VERSION,
    evaluate_ticker_policy,
)
from investment_panel.domain.decision.governance import (
    GOVERNANCE_STAGES,
    NOTIFIABLE_TRANSITIONS,
    OUTCOME_ERROR_TYPES,
    PROMOTION_STAGES,
    TRACKED_METRICS,
    classify_outcome_error,
    classify_outcome_evidence,
    promotion_readiness,
    transition_dedupe_key,
    valid_outcome_error_type,
)
from investment_panel.domain.decision.alpha import (
    ALPHA_SIGNAL_CONTRACT_VERSION,
    INSTRUMENT_STATE_SNAPSHOT_CONTRACT_VERSION,
    OPPORTUNITY_RANK_CONTRACT_VERSION,
    STRATEGY_FORECAST_CONTRACT_VERSION,
    TICKER_OPPORTUNITY_RANKING_VERSION,
    AlphaSignal,
    EligibleUniverseSnapshot,
    InstrumentStateSnapshot,
    OpportunityRank,
    StrategyForecast,
    TradeUtility,
    build_alpha_signal,
    build_instrument_state_snapshot,
    build_strategy_forecast,
    strategy_forecast_id_for_payload,
    calculate_trade_utility,
    rank_opportunities,
)
from investment_panel.domain.decision.ticker import AvailabilityStatus, availability_status_for_blockers
from investment_panel.domain.portfolio.risk_policy import RiskPolicySnapshot

__all__ = [
    "opportunity_rank_blocker",
    "trade_plan_rank_identity_matches",
    "ARCO_STALE_DAYS",
    "DAILY_ANALYSIS_SOURCES",
    "DAILY_STALE_DAYS",
    "FILING_STALE_DAYS",
    "FRESHNESS_ORDER",
    "GATE_LABELS",
    "INTRADAY_STALE_HOURS",
    "MARKET_CLOSE",
    "MARKET_OPEN",
    "MARKET_TZ",
    "PRIMARY_EVIDENCE_SOURCES",
    "STATIC_SOURCES",
    "SYMBOL_RE",
    "brief_summary",
    "is_no_trade_action",
    "is_option_expired",
    "jsonable",
    "action_grade_for",
    "GOVERNANCE_STAGES",
    "NOTIFIABLE_TRANSITIONS",
    "OUTCOME_ERROR_TYPES",
    "PROMOTION_STAGES",
    "TRACKED_METRICS",
    "classify_outcome_error",
    "classify_outcome_evidence",
    "promotion_readiness",
    "transition_dedupe_key",
    "valid_outcome_error_type",
    "apply_blocking_penalties",
    "best_freshness",
    "catalyst_window",
    "classify_freshness",
    "completed_trading_dates",
    "decode",
    "decision_basis",
    "dedupe_freshness",
    "default_freshness_detail",
    "easter_date",
    "eligibility_detail",
    "gate_reasons",
    "has_required_valuation_context",
    "invalidation_for",
    "is_market_open",
    "is_us_equity_early_close",
    "is_us_market_day",
    "last_weekday",
    "latest_by_symbol",
    "latest_completed_market_day",
    "market_deadline_passed",
    "valuation_mark_is_stale",
    "market_session_bounds",
    "market_session_elapsed",
    "normalized_utc",
    "nth_weekday",
    "observed_fixed_holiday",
    "overall_decision_freshness",
    "portfolio_impact",
    "readiness_blockers",
    "readiness_missing_inputs",
    "readiness_next_action",
    "readiness_portfolio_fit",
    "readiness_status",
    "recency_points",
    "related_symbols",
    "stale_after_label",
    "symbol_freshness_detail",
    "ticker_decision_brief",
    "top_source_cluster",
    "trading_day_lag",
    "us_market_holidays",
    "worst_freshness",
    "TICKER_DECISION_CONTRACT_VERSION",
    "MARKET_DIMENSIONS",
    "MARKET_HORIZONS",
    "MARKET_SOURCE_PRIORITY",
    "MARKET_REQUIRED_DIMENSIONS",
    "MARKET_REQUIRED_DIMENSIONS_BY_HORIZON",
    "OPPORTUNITY_EPISODE_CONTRACT_VERSION",
    "TRADE_EXPRESSION_CONTRACT_VERSION",
    "TRADE_PLAN_CONTRACT_VERSION",
    "OUTCOME_ATTRIBUTION_CONTRACT_VERSION",
    "OUTCOME_ATTRIBUTION_EVALUATION_VERSION",
    "CapitalAction",
    "CapitalActionType",
    "CoverageMatrix",
    "CoverageMatrixRow",
    "AuthorizationMode",
    "DataQuality",
    "DecisionResolutionV2",
    "DataRequest",
    "EvidenceItem",
    "EvidencePolarity",
    "ExpressionDecision",
    "ExpressionKind",
    "Horizon",
    "HorizonDecision",
    "InputManifest",
    "InputLineage",
    "Invalidation",
    "MarketDimensionState",
    "MarketEvidenceAssessment",
    "MarketStateSnapshot",
    "NumericRange",
    "OpportunityEpisodeStatus",
    "PriceRange",
    "RiskPolicy",
    "RiskPolicySnapshot",
    "PortfolioImpact",
    "RESOLUTION_CONTRACT_VERSION",
    "ResolutionAction",
    "ResolutionEligibility",
    "ResolutionLifecycle",
    "ResolutionProvenance",
    "ScenarioOutcome",
    "SignalDeclaration",
    "SignalEvidenceState",
    "Stance",
    "OpportunityEpisode",
    "TradeExpression",
    "TradePlan",
    "market_evidence_for_decision",
    "market_required_dimensions",
    "OutcomeAttribution",
    "OutcomeAttributionState",
    "OutcomeEvidence",
    "OutcomeEvidenceState",
    "PaperExecutionOutcome",
    "TickerDecision",
    "apply_opportunity_rank_safety",
    "bind_trade_plan",
    "build_opportunity_episode",
    "build_trade_plan",
    "build_ticker_decision",
    "expression_decision_from_trade_expression",
    "expression_decision_to_trade_expression",
    "opportunity_episode_from_legacy",
    "opportunity_episode_id",
    "trade_expression_from_expression_decision",
    "trade_expression_from_legacy",
    "trade_expression_identity",
    "outcome_attribution_id",
    "outcome_attribution_stable_key",
    "portfolio_impact_from_persisted",
    "portfolio_impact_id_for_persisted",
    "portfolio_impacts_from_persisted",
    "trade_plan_from_persisted",
    "build_decision_resolution",
    "capital_action_from_resolution",
    "next_action_for",
    "resolution_from_legacy",
    "resolution_from_historical",
    "resolution_from_published",
    "TICKER_LEARNING_VERSION",
    "evaluate_ticker_policy",
    "ALPHA_SIGNAL_CONTRACT_VERSION",
    "STRATEGY_FORECAST_CONTRACT_VERSION",
    "INSTRUMENT_STATE_SNAPSHOT_CONTRACT_VERSION",
    "OPPORTUNITY_RANK_CONTRACT_VERSION",
    "TICKER_OPPORTUNITY_RANKING_VERSION",
    "AlphaSignal",
    "StrategyForecast",
    "AvailabilityStatus",
    "availability_status_for_blockers",
    "EligibleUniverseSnapshot",
    "InstrumentStateSnapshot",
    "OpportunityRank",
    "TradeUtility",
    "build_alpha_signal",
    "build_instrument_state_snapshot",
    "build_strategy_forecast",
    "strategy_forecast_id_for_payload",
    "calculate_trade_utility",
    "rank_opportunities",
    "CoverageVector",
    "CoverageVectorRow",
    "DimensionPosterior",
    "EventObservation",
    "MarketStatePosterior",
    "PITObservation",
    "Phase2Source",
    "Phase2Status",
    "ScenarioPath",
    "assess_crypto_venue_data",
    "assess_option_oi_volume_sla",
    "build_coverage_vector",
    "build_market_state_posterior",
    "build_scenario_paths",
    "parse_coinmetrics_derivatives",
    "parse_corporate_expectations",
    "parse_event_consensus",
    "parse_fred_alfred",
    "parse_treasury_yield_curve",
    "posterior_can_influence_rank",
    "replay_scenario_path",
    "select_point_in_time",
    "source_status",
]
