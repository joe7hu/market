...
| `core/*.py` (leaf modules) | Domain helpers: `signals`, `sources`, `technicals`, `scoring`, `prices`, `fundamentals`, `portfolio`, `portfolio_intelligence`, `thesis_monitor`, `daily_brief`, `ibkr_options`, `options_intelligence`, `option_agent_runner`/`option_agent_postmortem`, `research`, `instruments`, `event_calendar`, `sec`, `arco`, `crypto`, `refresh_jobs` | that specific domain |
...
| `views/` | Feature view modules. Larger features are folders: `health/`, `optionsRadar/`. Shared: `rowFormat.ts`, `workspacePage.tsx` |
