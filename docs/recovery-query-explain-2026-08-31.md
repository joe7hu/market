...
the live PostgreSQL database with the production default `work_mem`. The
Funnel plans were recaptured with the API role's configured three-second
statement timeout; the other plans use a 30-second EXPLAIN session because
EXPLAIN analysis adds instrumentation overhead. Runtime API statements remain
