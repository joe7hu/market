...
| PG-04 | Real PostgreSQL test harness and coverage gates | done | PostgreSQL 18 fixtures; all 557 tests including slow tier pass; enforced live app/database coverage is 80.89% (minimum 80%); job/publication concurrency covered |
...
| PG-06 | Ingestion and normalized raw facts ported | done | Idempotent run/payload manifests, latest-only interactive quotes, monthly narrow option partitions, and active Robinhood/IBKR collectors ported; retired source collectors removed from installed scripts |
...
| PG-11 | Full tests, coverage, concurrency, and performance gates pass | done | 557 tests including slow tier; 80.89% enforced coverage; Ruff/architecture/TypeScript/build pass; 100 concurrent requests per route all 200 with p95 212ms status, 220ms `/today`, 349ms NVDA; restored DB counts/revision match; local DB is 11MB |
